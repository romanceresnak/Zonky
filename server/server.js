...
app.get("/api/users/auth", auth, (req, res) => {
  res.status(200).json({
    ...
    isManager: req.user.role === 2 ? true : false,
    isAuth: true,
...
app.get("/api/users/get_casual_users", (req, res) => {
  User.find({}, (err, casualusers) => {
    if (err) return res.status(400).send(err);
    res.status(200).send(casualusers);
  });
});
